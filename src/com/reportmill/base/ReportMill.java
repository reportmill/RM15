...
public class ReportMill {
    ...
    {
        setLicense(aLicense, false, isApp);
    }
    ...
    public static boolean checkString(String aString, boolean isApplication)
    {
        ...
        try {
            procCount = Integer.parseInt(prefix.substring(prefix.length() - 2, prefix.length() - 1), Character.MAX_RADIX);
        } catch (Exception e) {
            return false;
        }
        ...
        try {
            version = Integer.parseInt(prefix.substring(prefix.length() - 1), Character.MAX_RADIX);
        } catch (Exception e) {
