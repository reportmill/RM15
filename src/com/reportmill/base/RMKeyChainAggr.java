...
public class RMKeyChainAggr {
    ...
    public static Object getValue(Object aRoot, List aList, RMKeyChain aKeyChain)
    {
        ...
        if (aKeyChain.getOp() == RMKeyChain.Op.Chain && aKeyChain.getChildCount() > 0) {
            kchain = aKeyChain.getChildKeyChain(0);
            tail = aKeyChain.subchain(1);
        }
        ...
        if (kchain.getOp() == RMKeyChain.Op.Key) {
            ...
            if (method != null) {
                try {
                    return method.invoke(null, aList, tail);
                } catch (Exception e) {
                    System.err.println("RMKeyChainAggr: Failed to eval " + aKeyChain);
                }
            }
        }
        ...
        // Handle FunctionCall: if aggregate method, return result of aggregate with args (eval tail too, if present)
            ...
            if (method != null) {
                try {
                    val = method.invoke(null, aList, args);
                } catch (Exception e) {
                    System.err.println("RMKeyChainAggr: Failed to eval " + kchain);
                    val = null;
                }
                return tail != null ? RMKeyChain.getValue(aRoot, val, tail) : val;
            }
    ...
    private static Object invokeBuiltIn(String aKey, List aList, RMKeyChain aKeyChain)
    {
        ...
        switch (aKey) {
            case "total":
                return total(aList, aKeyChain);
            case "total2":
                return total2(aList, aKeyChain);
            case "totalX":
                return totalX(aList, aKeyChain);
            case "count":
                return count(aList, aKeyChain);
            case "countDeep":
                return countDeep(aList, aKeyChain);
            case "countUnique":
                return countUnique(aList, aKeyChain);
            case "average":
                return average(aList, aKeyChain);
            case "averageX":
                return averageX(aList, aKeyChain);
            case "min":
                return min(aList, aKeyChain);
            case "max":
                return max(aList, aKeyChain);
            case "get":
                return get(aList, aKeyChain);
            case "filter":
                return filter(aList, aKeyChain);
            case "group":
                return group(aList, aKeyChain);
            case "join":
                return join(aList, aKeyChain);
            case "listOf":
                return listOf(aList, aKeyChain);
            default:
    ...
    private static class MasterDetail implements RMKey.Get {
        ...
        {
            Object value = RMKey.getValue(_detail, aKey);
