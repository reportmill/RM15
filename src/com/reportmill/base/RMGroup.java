...
public class RMGroup extends ArrayList implements RMKey.Get, RMKeyChain.Get {
    ...
    {
        super(aList == null ? new ArrayList() : aList);
    }
    ...
    {
        return _pc >= 0 ? _pc : (_pc = pcimpl());
    }

    private final int pcimpl()
    {
        return _parent != null ? _parent.getParentCount() + 1 : 0;
    }
    ...
    public int index()
    {
        return ListUtils.indexOfId(_parent, this);
    }
    ...
    public RMGroup getGroup(int anIndex)
    {
        ...
        RMGroup group = (RMGroup) get(anIndex);
        if (group._parent != this) group._parent = this;
        return group;
    }
    ...
    {
        groupBy(aGrouping, getExplicitValues(aGrouping));
    }
    ...
    public void groupByKey(String aKey, List aValuesList)
    {
        ...
        if (aValuesList != null && aValuesList.size() > 0) {
            ...
            for (int i = 0, iMax = aValuesList.size(); i < iMax; i++) {
                Object value = aValuesList.get(i);
                ...
                if (valueKey == null) valueKey = Void.class;
        ...
        for (int i = start, iMax = size(); i < iMax; i++) {
            Object object = get(i);
            ...
            if (group == null) {
                group = new RMGroup();
                group._parent = this;
                group._value = value; // Create new group
    ...
    public void topNSortBy(RMTopNSort aSort)
    {
        ...
        if (aSort.getIncludeOthers() && othersLength > 0) {
            ...
            RMGroup group = new RMGroup();
            group._key = getGroup(othersStart).getKey();
            group._parent = this;
            group._isTopNOthers = true;

            // Iterate over others list and grab objects from other groups
            for (int i = othersStart; i < othersEnd; i++) {
                RMGroup other = getGroup(i);
    ...
    {
        return getAllValues(aKey, new HashSet(), new ArrayList());
    }
    ...
    public RMGroup subgroup(int start, int end)
    {
        ...
        group._key = _key;
        group._parent = _parent;
        group._isTopNOthers = _isTopNOthers;
        return group;
    }
     ...
     */
    public static boolean isLeaf(Object aList)
    {
        return aList instanceof RMGroup && ((RMGroup) aList).isLeaf();
    }

    /**
     * Returns whether the given List is an RMGroup instance that also top n others.
     */
    public static boolean isTopNOthers(Object aList)
    {
        return aList instanceof RMGroup && ((RMGroup) aList).isTopNOthers();
    }
    ...
    public Object getKeyChainValue(Object aRoot, RMKeyChain aKeyChain)
    {
        ...
        if (_isTopNOthers) { // && RMKeyChainAggr.getAggrMethod(aKC.getChildString(0))==null) {
            ...
            String kstr = aKeyChain.toString();
    ...
    private Object getFirstSample()
    {
        ...
        for (int i = 0, iMax = size(); i < iMax; i++) {
            Object child = get(i);
            if (child instanceof RMGroup) {
                RMGroup childGroup = (RMGroup) child;
    ...
    public RMGroup clone()
    {
        ...
        copy._isLeaf = _isLeaf;
        copy._isTopNOthers = _isTopNOthers;
        return copy;
    }
    ...
    public RMGroup cloneDeep()
    {
        ...
        for (int i = 0, iMax = size(); i < iMax; i++) {
            RMGroup grp = getGroup(i).cloneDeep();
            grp._parent = clone;
            clone.set(i, grp);
        }
    ...
    public String toString()
    {
        ...
        String pky = parent != null ? parent.getKey() : null;
        if (pky != null) {
            sb.append(", Parent.Key=").append(pky).append(", Value=").append(RMKeyChain.getValue(this, pky));
        }
        int index = index();
    ...
    public static class Running extends RMGroup {
        ...
        public Running(RMGroup aSourceGroup, RMGroup thePageStartGroup, RMGroup thePageEndGroup)
        {
            ...
            _pageStartGroup = thePageStartGroup;
            _pageEndGroup = thePageEndGroup;
