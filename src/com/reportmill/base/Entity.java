...
public class Entity implements XMLArchiver.Archivable {
    ...
    {
        return _schema != null ? _schema.getEntity(aName) : null;
    }
     ...
     */
    public void addProperty(Property... theProps)
    {
        for (Property p : theProps) addProperty(p);
    }
    ...
    {
        return getAttributes().get(anIndex);
    }
    ...
    public List<Property> getAttributes()
    {
        ...
        for (int i = 0, iMax = getPropertyCount(); i < iMax; i++)
            if (getProperty(i).isAttribute()) attrs.add(getProperty(i));
        return _attrs = attrs;
    }
    ...
    {
        return getRelations().get(anIndex);
    }
    ...
    {
        return getAttributesSorted().get(anIndex);
    }
    ...
    {
        return getRelationsSorted().get(anIndex);
    }
    ...
    {
        String pnames[] = aKeyPath != null ? aKeyPath.split("\\.") : new String[0];
        Property prop = null;
        for (String pname : pnames) {
            Entity entity = prop != null ? prop.getRelationEntity() : this;
            if (entity == null) return null;
            prop = entity.getProperty(pname);
            if (prop == null) break;
        }
        return prop;
    }
    ...
    public boolean equals(Object anObj)
    {
        ...
        Entity other = anObj instanceof Entity ? (Entity) anObj : null;
