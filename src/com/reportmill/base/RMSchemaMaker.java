...
public class RMSchemaMaker {
     ...
     */
    public void addIgnoreClass(String aClassName)
    {
        _ignoredClasses.add(aClassName);
    }

    /**
     * Tells schema maker to ignore any members encountered with the given name.
     */
    public void addIgnoreMember(String aName)
    {
        _ignoredMembers.add(aName);
    }
     ...
     */
    protected boolean ignoreMember(String aName)
    {
        return matches(aName, _ignoredMembers);
    }
    ...
    private boolean matches(String aName, Set<String> theNames)
    {
        ...
        for (String name : theNames) {
            ...
            if (name.startsWith("*")) {
                if (name.endsWith("*")) {
                    if (aName.contains(name.substring(1, name.length() - 1))) return true;
                } else if (name.endsWith(name.substring(1))) return true;
            }

            // If ends with wildcard, match with startsWith
            else if (name.endsWith("*")) {
                if (aName.startsWith(name.substring(0, name.length() - 1))) return true;
            }
    ...
    private void getSchema(XMLElement anElement, Schema aSchema)
    {
        ...
        for (int i = 0, iMax = anElement.getAttributeCount(); i < iMax; i++) {
            XMLAttribute attr = anElement.getAttribute(i);
        ...
        for (int i = 0, iMax = anElement.size(); i < iMax; i++) {
            XMLElement elmt = anElement.get(i);
