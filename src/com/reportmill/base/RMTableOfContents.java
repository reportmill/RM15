...
public class RMTableOfContents extends HashMap {
    ...
    private void findTable(RMShape aShape)
    {
        ...
        if (aShape instanceof RMTableRPG)
            ...
            // Otherwise, recurse
        else if (aShape instanceof RMParentShape) {
            RMParentShape parent = (RMParentShape) aShape;
    ...
    private void processTable(RMTableRPG aTable)
    {
        ...
        for (int i = 0, iMax = aTable.getChildCount(); i < iMax; i++) {
            RMTableRowRPG row = (RMTableRowRPG) aTable.getChild(i);
