...
public static void fitCurveFromPointIndex(RMPath path, int index)
{
    ...
    List points = new ArrayList(path.getPointCount()-index);
    for(int i=index, iMax=path.getPointCount(); i<iMax; i++) { Point point = path.getPoint(i);
    ...
    if(points.size()>1) {
        ...
        for(int i=0, iMax=beziers.size()/3; i<iMax; i++)
            path.curveTo(beziers.get(i*3+1), beziers.get(i*3+2), beziers.get(i*3+3));
    }
}
...
private static void FitCubic(List beziers, Point d[], int first, int last, Point tHat1, Point tHat2, double error)
{
    ...
    if(maxError < error) {
        ConcatBezierCurve(beziers, bezCurve); return; }
...
private static double NewtonRaphsonRootFind(Point bc[], Point P, double u)
{
    ...
    for(int i=0;i<=2;i++) Q1[i] = new Point((bc[i+1].x - bc[i].x)*3f, (bc[i+1].y - bc[i].y)*3f);
    for(int i=0;i<=1;i++) Q2[i] = new Point((Q1[i+1].x - Q1[i].x)*2f, (Q1[i+1].y - Q1[i].y)*2f);
...
{
    Point vtmp[] = new Point[4]; for(int i=0; i<=degree; i++) vtmp[i] = new Point(V[i]);
    for(int i=1; i<=degree; i++) for(int j=0; j<=degree-i; j++) {
...
{
    if(beziers.size()==0) beziers.add(bezCurve[0]);
    beziers.add(bezCurve[1]); beziers.add(bezCurve[2]); beziers.add(bezCurve[3]);
}
