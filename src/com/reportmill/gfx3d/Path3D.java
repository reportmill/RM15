...
public class Path3D extends Shape3D implements Cloneable {
     ...
     */
    public Seg getElement(int anIndex)
    {
        ...
        for (int i = 0, iMax = getElementCount(); i < iMax; i++) {
            ...
            switch (type) {
                ...
                case QuadTo:
                    ...
                    break;
                case CubicTo:
                    path.curveTo(pts[0].x, pts[0].y, pts[1].x, pts[1].y, pts[2].x, pts[2].y);
                    break;
                case Close:
                    path.close();
            }
    ...
    {
        return getZMin() + (getZMax() - getZMin()) / 2;
    }
    ...
    public static void sort(List<Path3D> paths)
    {
        ...
        for (int i = paths.size() - 1; i > 0; i--) {
            Path3D path1 = paths.get(i);
            int i2 = i;

            // Iterate over remaining paths
            for (int j = 0; j < i; j++) {
                Path3D path2 = paths.get(j);
                if (path2 == path1) continue;
                ...
                if (comp1 == ORDER_SAME || comp1 == ORDER_BACK_TO_FRONT) continue;
                int comp2 = path2.comparePlane(path1);
                if (comp2 == ORDER_FRONT_TO_BACK) continue;
