...
class RMGraphRPGBar extends RMGraphRPG {
    ...
    public RMGraphRPGBar(RMGraph aGraph, ReportOwner anRptOwner)
    {
        ...
        _meshed = isMeshed();
        _stacked = _graph.isStacked();
        _layered = _graph.isLayered();
    ...
    {
        return _intervals.getIntervalCount();
    }
    ...
    {
        return _intervals.getInterval(anIndex);
    }
    ...
    public void addBars()
    {
        ...
        for (int i = 0, iMax = getSeriesCount(); i < iMax; i++) {
            RMGraphSeries series = getSeries(i);
            for (int j = 0, jMax = series.getItemCount(); j < jMax; j++) {
                RMGraphSeries.Item seriesItem = series.getItem(j);
    ...
    public double getBarLabelMaxWidth()
    {
        // Get series count and series item count
        int seriesCount = getSeriesCount();
        if (seriesCount == 0) return _graph.getWidth();
    ...
    private void addLabelAxisLabels()
    {
        ...
        for (int i = 0, iMax = getSectionCount(); i < iMax; i++) {
            RMGraphSection section = getSection(i);
    ...
    {
        // Iterate over series
        for (int i = _graph.getSeriesCount() - 1; i >= 0; i--) {
            RMGraphPartSeries seriesPart = _graph.getSeries(i);
    ...
    {
        return _barProtype != null ? _barProtype : (_barProtype = createBarPrototype());
    }
