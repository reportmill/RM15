...
public class RMCrossTabRow extends RMCrossTabSpan implements XMLArchiver.Archivable {
    ...
    {
        double y = 0;
        for (int i = 0, iMax = getIndex(); i < iMax; i++) y += _table.getRow(i).getHeight();
        return y;
    }
    ...
    public List<RMCrossTabDivider> getDividers()
    {
        ...
        divider.setBounds(0, Math.round(getMaxY()), _table.getWidth(), 0);
        divider._start = 0;
        ...
        for (int i = 0, iMax = _table.getColCount(); i < iMax; i++) {
            ...
            if ((cell != null && cell.getRowEnd() != row) || !showBorder) {
                ...
                if (divider != null) {
                    divider.setWidth(cellX - divider.getX());
                    divider._end = i;
                }
    ...
    {
        return (RMCrossTabRow) super.clone();
    }
    ...
    {
        return "RMCrossTabRow: height=" + getHeight() + ", cells=" + _cells.toString();
    }
