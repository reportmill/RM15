...
public class RMTableRPG extends RMParentShape {
    ...
    protected RMTableRPG addPage()
    {
        ...
        RMTableRPG npage = _nextPage = createPage();
        npage._table = _table;
        npage._page = _page + 1;
        npage._pageBreak = _pageBreak;
        npage._pageBreakPage = _pageBreakPage + 1;
        if (_doPageBreak) {
            npage._pageBreak++;
            npage._pageBreakPage = 1;
        }
        return npage;
    }
    ...
    {
        return new RMTableRPG(_rptOwner, _table);
    }
    ...
    protected boolean addRows(RMGroup aGroup, RMTableRowRPG aParentRPG, RMTableRowRPG theLastRow)
    {
        ...
        if (_doPageBreak) {
            _lastRow = new RMTableRowRPG();
            _lastRow._group = aGroup;
            return false;
        }
        ...
        if (headerRow != null && (!aGroup.isEmpty() || headerRow.getPrintEvenIfGroupIsEmpty())) {
            ...
            if (rowRPG == null) {
                rowRPG = new RMTableRowRPG();
                rowRPG.rpgAll(_rptOwner, headerRow, aGroup, version);
            }
        ...
        int added = 0;
        for (int i = 0, iMax = aGroup.size(); i < iMax; i++, added++) {
            RMGroup childGroup = aGroup.getGroup(i);
            ...
            if (theLastRow != null && !theLastRow._group.isAncestor(childGroup)) {
                if (theLastRow._group == childGroup) {
                    if (theLastRow._row == null) theLastRow = null;
                } else continue;
            }
            ...
            if (detailsRow != null) {
                ...
                if (theLastRow != null) {
                    if (theLastRow._row == detailsRow) {
                        ...
                        rowRPG = new RMTableRowRPG();
                        rowRPG._row = detailsRow;
                        rowRPG._group = childGroup;
                    } else version = RMTableRow.VersionReprint;
                }
            ...
            if (_paginating && i + 1 < iMax) {
                ...
                if (_doPageBreak) {
                    _lastRow = new RMTableRowRPG();
                    _lastRow._group = aGroup.getGroup(i + 1);
                    added++;
                    break;
                }
            }
        ...
        if (added < aGroup.size()) {
            ...
            RMGroup pageStartRowGroup = aGroup;
            int pageStartRowIndex = 0;
            for (int i = 0, iMax = getChildCount(); i < iMax; i++) {
                RMTableRowRPG row = (RMTableRowRPG) getChild(i);
                if (row.getGroup().getParentCount() > pageStartRowGroup.getParentCount()) {
                    pageStartRowGroup = row.getGroup();
                    pageStartRowIndex = i;
                }
            }
            ...
            while (getChildCount() > pageStartRowIndex) {
                RMTableRowRPG oldLastRow = _lastRow;
                _lastRow = (RMTableRowRPG) getChildLast();
                RMGroup group = new RMGroup.Running(aGroup, pageStartRowGroup, oldLastRow.getGroup());
                RMTableRowRPG row = new RMTableRowRPG();
                row.rpgAll(_rptOwner, summaryRow, group, "Running");
    ...
    boolean addRow(RMTableRowRPG aRow, RMTableRowRPG aParentRPG)
    {
        ...
        while (parentRPG != _topRow && !isSatisfied(parentRPG, -1)) {
            lastRow = parentRPG;
            parentRPG = parentRPG._parentRPG;
        }
    ...
    boolean isSatisfied(RMTableRowRPG aParentRPG, int aMod)
    {
        ...
        boolean tops = true;
        for (RMTableRowRPG p = aParentRPG._parentRPG; p != null && tops; p = p._parentRPG)
            tops = p.getChildRPGCount() == 1;
        return tops;
    }
    ...
    boolean splitRow(RMTableRowRPG aRow)
    {
        ...
        RMTableRowRPG split = (RMTableRowRPG) aRow.divideShapeFromTop(part1Height);
        aRow._split = split;
        split.setBestHeight();
        split.layoutDeep();
        return true;
    ...
    protected void moveRowsToBottom()
    {
        ...
        double shift = 0;
        for (int i = 0, iMax = getChildCount(); i < iMax; i++) {
            RMTableRowRPG rowRPG = (RMTableRowRPG) getChild(i);
            if (shift == 0 && rowRPG._row.getMoveToBottom()) {
                shift = getHeight() - getChildLast().getFrameMaxY();
            }
    ...
    protected RMTableRPG makeColumns()
    {
        ...
        while (page != null) {
            ...
            if (page != null) {
                cpage._nextPage = new RMTableRPG(_rptOwner, null);
                cpage = cpage._nextPage;
    ...
    {
        return _nextPage != null ? _nextPage.getPageBreakMax() : _pageBreak;
    }
    ...
    {
        return _prefHeight > 0 ? _prefHeight : getHeight();
    }
