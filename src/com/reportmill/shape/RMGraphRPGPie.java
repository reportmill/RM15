...
class RMGraphRPGPie extends RMGraphRPG {
    ...
    {
        if (_graph.getKeyCount() == 1) return false;
        return super.isMeshed();
    }
        ...
        for (int i = 0, iMax = getSectionCount(); i < iMax; i++) {
            ...
            for (int j = 0, jMax = section.getItemCount(); j < jMax; j++) {
                ...
                if (extrude) {
                    ...
                    Rect wedgeBounds = pieBounds.clone();
                    wedgeBounds.offset(offsetX, offsetY);
    ...
    public static class PieGraphShape2D extends RMParentShape implements PieGraphShape {
         ...
         */
        public PieGraphShape2D(RMGraph aGraph)
        {
            copyShape(aGraph);
        }
