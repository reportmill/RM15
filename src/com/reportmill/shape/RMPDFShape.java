...
public class RMPDFShape extends RMRectShape {
     ...
     */
    public RMPDFShape(Object aSource)
    {
        setPDFData(aSource);
        setBestSize();
    }
     ...
     */
    public void setPDFData(Object aSource)
    {
        setPDFData(RMPDFData.getPDFData(aSource));
    }
    ...
    {
        int index = anIndex;
        if (getPDFData() != null) index = MathUtils.clamp(index, 0, getPDFData().getPageCount() - 1);
    ...
    {
        RMPDFShape pshape = new RMPDFShape();
        pshape.copyShape(aShape);
        pshape.setGrowToFit(aShape.isGrowToFit());
        pshape.setPreserveRatio(aShape.getPreserveRatio());
        pshape.setPadding(aShape.getPadding());
        pshape.setPDFData(aSource);
        pshape.setPrefHeight(pshape.getHeight() * pshape.getScaleY());
        return pshape;
    }
    ...
    protected void paintShape(Painter aPntr)
    {
        ...
        RMPDFData pd = getPDFData();
        if (pd == null) {
            System.out.println("RMPDFShape.paint: Need empty impl");
            return;
        }
    ...
    public static RMDocument getDocPDF(Object aSource, RMDocument aBaseDoc)
    {
        ...
        for (int i = 0, iMax = pdata.getPageCount(); i < iMax; i++) {
            RMPDFData pd = pdata.getPage(i);
            RMPage page = doc.addPage();
            page.setSize(pd.getWidth(), pd.getHeight());
