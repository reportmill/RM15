...
public class RMShapeUtils {
    ...
    {
        double w = 0;
        for (RMShape s : theShapes) w += s.getWidth();
    ...
    {
        double my = 0;
        for (RMShape s : theShapes) if (s.isVisible()) my = Math.max(my, s.getFrameMaxY());
        return my;
    }
    ...
    {
        int c = compareShapeFrameY(s0, s1);
        if (c == 0) c = compareShapeFrameX(s0, s1);
        return c;
    }
    ...
    public static RMPolygonShape getCombinedPathsShape(List<RMShape> theShapes)
    {
        ...
        RMShape shape0 = theShapes.size() > 0 ? theShapes.get(0) : null;
        if (shape0 == null) return null;
    ...
    {
        List<Shape> paths = getPathsFromShapes(theShapes, 0);
    ...
    public static RMPolygonShape getSubtractedPathsShape(List<RMShape> theShapes, int anInset)
    {
        ...
        shape.copyShape(theShapes.get(0));
        shape._rss = null;
    ...
    public static Shape getSubtractedPath(List<RMShape> theShapes, int anInset)
    {
        ...
        List<Shape> paths = getPathsFromShapes(shapes, anInset);
        Shape s1 = paths.get(0);
        for (int i = 1, iMax = paths.size(); i < iMax; i++) {
            Shape s2 = paths.get(i);
