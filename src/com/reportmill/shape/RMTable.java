...
public class RMTable extends RMParentShape {
    ...
    {
        if (_grouper == null) setGrouper(new RMGrouper());
        return _grouper;
    }
    ...
    {
        return getGrouper().getGrouping(anIndex);
    }
    ...
    {
        return getGrouper().getGrouping(aKey);
    }
    ...
    {
        getGrouping(aKey, true, anIndex).setHasDetails(true);
    }
    ...
    {
        getGrouper().addGrouping(aGrouping, anIndex);
    }
    ...
    {
        getGrouper().removeGrouping(aGrouping);
    }
    ...
    {
        getGrouper().moveGrouping(fromIndex, toIndex);
    }
    ...
    {
        getGrouping(aKey, true, 0).setHasHeader(true);
        return getRow(aKey + " Header");
    }
    ...
    {
        getGrouping(aKey, true, 0).setHasDetails(true);
        return getRow(aKey + " Details");
    }
    ...
    {
        getGrouping(aKey, true, 0).setHasSummary(true);
        return getRow(aKey + " Summary");
    }
    ...
    protected void grouperChanged()
    {
        ...
        for (int i = 0, iMax = getGroupingCount(); i < iMax; i++) {
            RMGrouping grouping = getGrouping(i);
        ...
        for (int i = getGroupingCount() - 1; i >= 0; i--) {
            RMGrouping grouping = getGrouping(i);
    ...
    {
        return (RMTableRow) getChild(anIndex);
    }
    ...
    {
        return (RMTableRow) getChildWithTitle(aName);
    }
    ...
    {
        RMTableRow r = getRow(aNm1);
        if (r != null) r.setTitle(aNm2);
    }
     ...
     */
    public Rect getResizeBarBounds(int anIndex)
    {
        return new Rect(0, getChild(anIndex).getMaxY(), getWidth(), 16);
    }
    ...
    protected void paintShape(Painter aPntr)
    {
        ...
        aPntr.setColor(new Color(11 / 12f));
        aPntr.fillRect(0, y, getWidth(), bounds.getMaxY() - y);
        ...
        for (int i = 0, iMax = getChildCount(); i < iMax; i++) {
            ...
            // If not super-selected, draw resize bar flat with gray border
                ...
                aPntr.fill(resizeBarBounds);
                aPntr.setColor(Color.DARKGRAY);
                aPntr.draw(resizeBarBounds);
            }
            ...
            if (drawSuperSelect) {
                ...
                if (props.isSelected(child) || props.isSuperSelected(child)) {
                    ...
                    if (child.isStructured())
                        for (int j = 0, jMax = child.getChildCount(); j < jMax; j++) {
                            RMShape trChild = child.getChild(j);
    ...
    {
        return getClass().getSimpleName() + " " + getDatasetKey() + " " + getFrame().toString();
    }
