...
public class RMCrossTabCol extends RMCrossTabSpan implements XMLArchiver.Archivable {
    ...
    {
        double x = 0;
        for (int i = 0, iMax = getIndex(); i < iMax; i++) x += _table.getCol(i).getWidth();
        return x;
    }
    ...
    public double getBestWidth()
    {
        double pw = getWidth();
        for (int i = 0, iMax = getCellCount(); i < iMax; i++) {
            RMCrossTabCell cell = getCell(i);
            if (cell.getColSpan() > 1) continue;
            pw = Math.max(pw, cell.getBestWidth());
        }
        return pw;
    }
    ...
    public List<RMCrossTabDivider> getDividers()
    {
        ...
        divider.setBounds(Math.round(getMaxX()), 0, 0, _table.getHeight());
        divider._start = 0;
        ...
        for (int i = 0, iMax = _table.getRowCount(); i < iMax; i++) {
            ...
            if ((cell != null && cell.getColEnd() != column) || !showBorder) {
                ...
                if (divider != null) {
                    divider.setHeight(cellY - divider.getY());
                    divider._end = i;
                }
    ...
    {
        return (RMCrossTabCol) super.clone();
    }
    ...
    {
        return "RMCrossTabCol: width=" + getWidth() + ", cells=" + _cells.toString();
    }
