...
public class RMTableRow extends RMSwitchShape {
    ...
    {
        return ClassUtils.getInstance(getParent(), RMTable.class);
    }
    ...
    {
        return _title != null ? _title : "UnknownTitle";
    }
    ...
    public void setStructured(boolean aFlag)
    {
        ...
        else for (int i = getChildCount() - 1; i >= 0; i--) {
    ...
    {
        return _syncStructureWithRowAbove;
    }
    ...
    {
        return _syncStructureWithAlternates;
    }
    ...
    {
        return ClassUtils.getInstance(getChild(anIndex), RMTextShape.class);
    }
    ...
    {
        return _numberOfChildrenToStayWith;
    }
    ...
    {
        return _printEvenIfNoObjectsInGroup;
    }
    ...
    {
        return _minSplitRemainderHeight;
    }
     ...
     */
    public String getPageBreakKey()
    {
        return _pageBreakKey;
    }
    ...
    {
        return _deleteVerticalSpansOfHiddenShapes;
    }
    ...
    {
        return _shiftShapesBelowHiddenShapesUp;
    }
    ...
    public void syncStructureWithShape(RMParentShape aShape)
    {
        ...
        if (aShape.getChildCount() > 0 && aShape.getChildCount() == getChildCount()) {
            ...
            for (int i = 0, iMax = children.size(); i < iMax; i++) {
                RMShape child = children.get(i), schild = schildren.get(i);
                child._x = schild._x;
                child.setWidth(schild._width);
            }
        }
    ...
    public RMTableRow getRowAbove()
    {
        ...
        int index = indexOf();
        if (index <= 0 || getTable() == null) return null;
    ...
    protected void paintShape(Painter aPntr)
    {
        ...
        if (!RMShapePaintProps.isEditing(aPntr) || !isStructured()) return;
    ...
    {
        return !isStructured() ? getBoundsInside() : null;
    }
    ...
    protected void layoutImpl()
    {
        ...
        List<RMShape> children = getChildren();
        double width = 0;
        for (RMShape child : children) {
        ...
        if (!MathUtils.equals(width, pwidth)) {
            double extra = pwidth - width, x = 0;
            for (RMShape child : children) {
    ...
    protected void fromXMLShape(XMLArchiver anArchiver, XMLElement anElement)
    {
        ...
        if (anElement.getElement("layout") != null) {
            XMLElement lxml = anElement.removeElement("layout");
