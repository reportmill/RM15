...
public class RMCrossTab extends RMParentShape {
    ...
    public RMCrossTab()
    {
        _topBrdrRow._table = _leftBrdrCol._table = this;
        _topBrdrRow._height = 0;
        _leftBrdrCol._width = 0;
    }
     ...
     */
    public void setDatasetKey(String aKey)
    {
        _datasetKey = aKey;
    }
    ...
    {
        return getRow(aRow).getCell(aColumn);
    }
    ...
    protected void setCell(RMCrossTabCell aCell, int aRow, int aCol, int aRowSpan, int aColSpan)
    {
        ...
        aCell._rowSpan = aRowSpan;
        aCell._colSpan = aColSpan;
        for (int i = 0; i < aRowSpan; i++)
            for (int j = 0; j < aColSpan; j++) {
    ...
    private RMCrossTabCell createCell(int row, int column, RMCrossTabCell aRefCell)
    {
        ...
        if (row < getHeaderRowCount() && column < getHeaderColCount())
            ...
            cell.setColor(HEADER_ROW_COLOR);
            if (!cell.getFont().isBold()) cell.setFont(cell.getFont().getBold());
        }

        // If adding header column, set header column attributes
        else if (column < getHeaderColCount()) {
            cell.setColor(HEADER_COLUMN_COLOR);
            if (!cell.getFont().isBold()) cell.setFont(cell.getFont().getBold());
        }
    ...
    {
        while (_headerRowCount < aCount) {
            _headerRowCount++;
            addRow(_headerRowCount - 1);
        } // If too few, add new
    ...
    {
        while (_headerColCount < aCount) {
            _headerColCount++;
            addCol(_headerColCount - 1);
        } // If too few, add new
    ...
    void validateCells()
    {
        ...
        int rcount = getRowCount(), ccount = getColCount();
        if (rcount == 0 || ccount == 0) return;
        for (int i = 0, iMax = rcount; i < iMax; i++)
            for (int j = 0, jMax = ccount; j < jMax; j++) getCell(i, j)._row = -1;
        ...
        for (int i = 0, iMax = rcount; i < iMax; i++) {
            for (int j = 0, jMax = ccount; j < jMax; j++) {
                ...
                cell._parent = this;
                int rspan = 1;
                while (j + rspan < ccount && getCell(i, j + rspan) == cell) rspan++; // Calculate row span
                int cspan = 1;
                while (i + cspan < rcount && getCell(i + cspan, j) == cell) cspan++; // Calculate col span
                cell._colSpan = rspan;
                cell._rowSpan = cspan; // Set rspan/cspan
            }
        }
    }
    ...
    protected void layoutImpl()
    {
        ...
        for (int i = 0, iMax = rcount; i < iMax; i++) {
            for (int j = 0, jMax = ccount; j < jMax; j++) {
                ...
                double width = 0;
                for (int k = 0, kMax = cell.getColSpan(); k < kMax; k++) width += getCol(j + k).getWidth();
                double height = 0;
                for (int k = 0, kMax = cell.getRowSpan(); k < kMax; k++) height += getRow(i + k).getHeight();
        ...
        _topBrdrRow.resetDividers();
        _leftBrdrCol.resetDividers();
        for (int j = 0, jMax = _topBrdrRow.getDividerCount(); j < jMax; j++) {
            RMCrossTabDivider div = _topBrdrRow.getDivider(j);
            if (div.getLength() > 0) addChild(div);
        }
        for (int j = 0, jMax = _leftBrdrCol.getDividerCount(); j < jMax; j++) {
            RMCrossTabDivider div = _leftBrdrCol.getDivider(j);
            if (div.getLength() > 0) addChild(div);
        }
        ...
        for (int i = 0, iMax = rcount; i < iMax; i++) {
            RMCrossTabRow row = getRow(i);
            row.resetDividers();
            for (int j = 0, jMax = row.getDividerCount(); j < jMax; j++) {
                RMCrossTabDivider div = row.getDivider(j);
                if (div.getLength() > 0) addChild(div);
            }
        }

        // Iterate over columns and add column dividers
        for (int i = 0, iMax = ccount; i < iMax; i++) {
            RMCrossTabCol col = getCol(i);
            col.resetDividers();
            for (int j = 0, jMax = col.getDividerCount(); j < jMax; j++) {
                RMCrossTabDivider div = col.getDivider(j);
                if (div.getLength() > 0) addChild(div);
            }
        }
    }
    ...
    {
        double cw = 0;
        for (int i = 0, iMax = getColCount(); i < iMax; i++) cw += getCol(i).getWidth();
        double w = getWidth();
        if (MathUtils.equals(w, cw)) return;
        double dw = w - cw;
        int cc = getColCount(), incr = 1;
        if (dw < 0) {
            dw = -dw;
            incr = -1;
            if (w < cc * 8) return;
        ...
        while (dw > 0) {
            RMCrossTabCol c = getCol(Math.abs(_ci % cc));
            _ci += incr;
            if (incr < 0 && c._width < 8) continue;
            if (dw >= 1) {
                c._width += incr;
                dw -= 1;
            } else {
    ...
    {
        double rh = 0;
        for (int i = 0, iMax = getRowCount(); i < iMax; i++) rh += getRow(i).getHeight();
        double h = getHeight();
        if (MathUtils.equals(h, rh)) return;
        double dh = h - rh;
        int rc = getRowCount(), incr = 1;
        if (dh < 0) {
            dh = -dh;
            incr = -1;
            if (h < rc * 8) return;
        ...
        while (dh > 0) {
            RMCrossTabRow r = getRow(Math.abs(_ri % rc));
            _ri += incr;
            if (incr < 0 && r._height < 8) continue;
            if (dh >= 1) {
                r._height += incr;
                dh -= 1;
            } else {
    ...
    public RMShape divideShapeFromTop(double anAmount)
    {
        ...
        RMCrossTab bottom = clone();
        bottom._rows = new ArrayList();
        bottom._height = 0;

        // Add Header rows
        if (reprintHeaderRows) {
            for (int i = 0, iMax = getHeaderRowCount(); i < iMax; i++) {
                RMCrossTabRow hrow = getRow(i);
        ...
        for (int j = 0, jMax = getColCount(); j < jMax; j++) {
            RMCrossTabCell cell = row.getCell(j);
            if (cell.getRow() != rowIndex) {
                RMCrossTabCell clone = cell.clone();
    ...
    public RMCrossTab clone()
    {
        ...
        clone._cols = new ArrayList();
        for (RMCrossTabCol col : _cols) clone._cols.add(col.clone());
        clone._rows = new ArrayList();
        for (RMCrossTabRow row : _rows) clone._rows.add(row.clone());
        ...
        clone._topBrdrRow = new RMCrossTabRow();
        clone._topBrdrRow._table = clone;
        clone._topBrdrRow._height = 0;
        clone._leftBrdrCol = new RMCrossTabCol();
        clone._leftBrdrCol._table = clone;
        clone._leftBrdrCol._width = 0;
        return clone;
    }
