...
public class RMShape implements Cloneable, RMTypes, Archivable, Key.GetSet {
    ...
    public double getY()
    {
        return _height < 0 ? _y + _height : _y;
    }
     ...
     */
    public void setFrame(Rect aRect)
    {
        setFrame(aRect.getX(), aRect.getY(), aRect.getWidth(), aRect.getHeight());
    }
    ...
    {
        return isRSS() ? getFrameXY().x : getX();
    }
    ...
    {
        return isRSS() ? getFrameXY().y : getY();
    }
    ...
    public void setFrameSize(double aWidth, double aHeight)
    {
        ...
        if (!isRSS()) {
            if (_width < 0) {
                setX(_x + (aWidth + _width));
                aWidth = -aWidth;
            }
            if (_height < 0) {
                setY(_y + (aHeight + _height));
                aHeight = -aHeight;
            }
    ...
    public Rect getBoundsStroked()
    {
        ...
        if (stroke == null) return bnds;
        bnds.inset(-stroke.getWidth() / 2);
        return bnds;
    }
    ...
    {
        aValue = Math.round(aValue * 100) / 100d;
     ...
     */
    protected double[] getRSS()
    {
        return _rss != null ? _rss : (_rss = new double[]{0, 1, 1, 0, 0});
    }
    ...
    {
        return _asize != null ? _asize : getAutosizingDefault();
    }
    ...
    public void setAutosizing(String aValue)
    {
        if (aValue != null && (aValue.length() < 7 || !(aValue.charAt(0) == '-' || aValue.charAt(0) == '~'))) {
            System.err.println("RMShape.setAutosizing: Invalid string: " + aValue);
            return;
        }
    ...
    {
        return isVisible() && (_parent == null || _parent.isHittable(this));
    }
    ...
    public void setAlignment(Pos aPos)
    {
        switch (aPos.getHPos()) {
            case LEFT:
                setAlignmentX(AlignX.Left);
                break;
            case CENTER:
                setAlignmentX(AlignX.Center);
                break;
            case RIGHT:
                setAlignmentX(AlignX.Right);
                break;
        }
        switch (aPos.getVPos()) {
            case TOP:
    ...
    {
        return null;
    } //if(getBindingCount()>0) return (RMFormat)getBinding(0).getFormat();
    ...
    {
        return SnapUtils.boolValue(get("Locked"));
    }
    ...
    {
        return _parent != null ? _parent.getRootShape() : this;
    }
     ...
     */
    public Point parentToLocal(double aX, double aY, RMShape aPar)
    {
        return getParentToLocal(aPar).transformXY(aX, aY);
    }

    /**
     * Converts a point from parent to local.
     */
    public Point parentToLocal(Point aPoint)
    {
        return parentToLocal(aPoint.x, aPoint.y);
    }

    /**
     * Converts a point from given parent to local.
     */
    public Point parentToLocal(Point aPoint, RMShape aPar)
    {
        return parentToLocal(aPoint.x, aPoint.y, aPar);
    }
    ...
    public Transform getTransform()
    {
        ...
        Transform t = new Transform(getX(), getY());
        if (!isRSS()) return t;
    ...
    {
        Double w = (Double) get(MinWidth_Prop);
        return w != null ? w : 0;
    }
    ...
    {
        double w = aWidth <= 0 ? 0 : aWidth;
        if (w == getMinWidth()) return;
    ...
    {
        return get(MinHeight_Prop) != null;
    }
    ...
    {
        Double h = (Double) get(MinHeight_Prop);
        return h != null ? h : 0;
    }
    ...
    {
        return get(PrefWidth_Prop) != null;
    }
    ...
    {
        Double v = (Double) get(PrefWidth_Prop);
        if (v != null) return v;
    ...
    {
        return get(PrefHeight_Prop) != null;
    }
    ...
    {
        Double v = (Double) get(PrefHeight_Prop);
        if (v != null) return v;
    ...
    {
        return Math.max(getMinWidth(), getPrefWidth());
    }
    ...
    {
        return Math.max(getMinHeight(), getPrefHeight());
    }
    ...
    public Entity getDatasetEntity()
    {
        ...
        Entity parentEntity = parent.getDatasetEntity();
    ...
    {
        return getBindings(true).get(anIndex);
    }
    ...
    {
        return getBindings(true).remove(anIndex);
    }
    ...
    {
        addBinding(new Binding(aPropName, aKey));
    }
    ...
    public void rpgBindings(ReportOwner anRptOwner, RMShape aShapeRPG)
    {
        ...
        for (int i = 0; i < getBindingCount(); i++) {
            ...
            String pname = binding.getPropertyName();
            ...
            if (pname.equals("Font")) {
                ...
                String fs = value instanceof String ? (String) value : null;
    ...
    {
        return getClass() == RMParentShape.class;
    }
    ...
    public int page()
    {
        return _parent != null ? _parent.page() : 0;
    }
    ...
    public int pageMax()
    {
        return _parent != null ? _parent.pageMax() : 0;
    }
    ...
    {
        return _parent != null ? _parent.getPageBreak() : 0;
    }
    ...
    {
        return _parent != null ? _parent.getPageBreakMax() : 0;
    }
    ...
    {
        return _parent != null ? _parent.getPageBreakPage() : 0;
    }
    ...
    {
        return _parent != null ? _parent.getPageBreakPageMax() : 0;
    }
    ...
    public Object getKeyValue(String aPropName)
    {
        ...
        switch (aPropName) {
            ...
            case PrefWidth_Prop:
                return getPrefWidth();
            case PrefHeight_Prop:
                return getPrefHeight();
            default:
                return Key.getValueImpl(this, aPropName);
        }
    }
    ...
    public void setKeyValue(String aPropName, Object aValue)
    {
        ...
        switch (aPropName) {
            case X_Prop:
                setX(SnapUtils.doubleValue(aValue));
                break;
            case Y_Prop:
                setY(SnapUtils.doubleValue(aValue));
                break;
            case Width_Prop:
                setWidth(SnapUtils.doubleValue(aValue));
                break;
            case Height_Prop:
                setHeight(SnapUtils.doubleValue(aValue));
                break;
            case Roll_Prop:
                setRoll(SnapUtils.doubleValue(aValue));
                break;
            case ScaleX_Prop:
                setScaleX(SnapUtils.doubleValue(aValue));
                break;
            case ScaleY_Prop:
                setScaleY(SnapUtils.doubleValue(aValue));
                break;
            case SkewX_Prop:
                setSkewX(SnapUtils.doubleValue(aValue));
                break;
            case SkewY_Prop:
                setSkewY(SnapUtils.doubleValue(aValue));
                break;
            case Stroke_Prop:
                setStroke(aValue instanceof RMStroke ? (RMStroke) aValue : null);
                break;
            case Fill_Prop:
                setFill(aValue instanceof RMFill ? (RMFill) aValue : null);
                break;
            case Effect_Prop:
                setEffect(aValue instanceof Effect ? (Effect) aValue : null);
                break;
            case Opacity_Prop:
                setOpacity(SnapUtils.doubleValue(aValue));
                break;
            case Name_Prop:
                setName(SnapUtils.stringValue(aValue));
                break;
            case Visible_Prop:
                setVisible(SnapUtils.boolValue(aValue));
                break;
            case Locked_Prop:
                setLocked(SnapUtils.boolValue(aValue));
                break;
            case MinWidth_Prop:
                setMinWidth(SnapUtils.doubleValue(aValue));
                break;
            case MinHeight_Prop:
                setMinHeight(SnapUtils.doubleValue(aValue));
                break;
            case PrefWidth_Prop:
                setPrefWidth(SnapUtils.doubleValue(aValue));
                break;
            case PrefHeight_Prop:
                setPrefHeight(SnapUtils.doubleValue(aValue));
                break;
            default:
                Key.setValueReflectSafe(this, aPropName, aValue);
        }
    }
    ...
    public Object fromXML(XMLArchiver anArchiver, XMLElement anElement)
    {
        ...
        for (int i = anElement.indexOf("binding"); i >= 0; i = anElement.indexOf("binding", i + 1)) {
            XMLElement bxml = anElement.get(i);
        ...
        for (int i = anElement.indexOf("property-key"); i >= 0; i = anElement.indexOf("property-key", i + 1)) {
            XMLElement prop = anElement.get(i);
            String name = prop.getAttributeValue("name");
            if (name.equals("FontColor")) name = "TextColor";
            if (name.equals("IsVisible")) name = "Visible";
            String key = prop.getAttributeValue("key");
            addBinding(new Binding(name, key));
        }
    ...
    private static class RMSharedMap extends HashMap {
         ...
         */
        public RMSharedMap cloneReal()
        {
            RMSharedMap cln = (RMSharedMap) super.clone();
            cln.isShared = false;
            return cln;
        }
    }
