...
public class RMGraphLegend extends RMParentShape {
    ...
    protected void configureRPG(RMGraphRPG graphRPG, boolean doRPG)
    {
        ...
        if (graphRPG == null) {
            System.err.println("RMGraphLegend.confiureRPG: Graph RPG not found");
            return;
        }

        // Get Graph
        RMGraph graph = getGraph();
        if (graph == null) graph = graphRPG._graph;
     ...
     */
    private RMGraph getGraph()
    {
        return getParent() != null ? getParent().getChildWithClass(RMGraph.class) : null;
    }
