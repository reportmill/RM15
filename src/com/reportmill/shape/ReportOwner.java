...
public class ReportOwner implements RMKeyChain.Get {
    ...
    public void addModelObject(Object anObj)
    {
        ...
        if (obj instanceof List) //_model.put("RMDefaultObjectList", obj);
            ...
            Map map = (Map) obj;
            map = RMSQLUtils.getMapsDeep(map, 2);
            _model.putAll(map);
        }
    ...
    {
        return _dataStack.remove(_dataStack.size() - 1);
    }
    ...
    {
        return ListUtils.getLast(_dataStack);
    }
    ...
    {
        ListUtils.addUniqueId(_pageRefShapes, aShape);
    }
    ...
    public Object getKeyChainValue(Object aRoot, RMKeyChain aKeyChain)
    {
        ...
        if (op == RMKeyChain.Op.Key || op == RMKeyChain.Op.Chain || op == RMKeyChain.Op.FunctionCall) {
            ...
            for (int i = _dataStack.size() - 1; i >= 0; i--) {
                Object dso = _dataStack.get(i);
            ...
            if (op == RMKeyChain.Op.Key) {
                String key = aKeyChain.getValueString();
    ...
    public void resolvePageReferences()
    {
        ...
        for (int i = 0, iMax = prshapes.size(); i < iMax; i++) {
            RMShape shape = prshapes.get(i);
