...
public class RMTableRowRPG extends RMSpringShape {
    ...
    {
        return _row.getTitle().endsWith("Header");
    }
    ...
    {
        return _row.getTitle().endsWith("Details");
    }
    ...
    {
        return _row.getTitle().endsWith("Summary");
    }
    ...
    String getVersionName(ReportOwner anRptOwner, RMTableRow aRow, RMGroup aGroup, String aSuggVersion)
    {
        ...
        if (aGroup.isTopNOthers()) {
            if (RMTableRow.VersionReprint.equals(aSuggVersion) && aRow.hasVersion("TopN Others Reprint")) {
                aGroup.setTopNOthers(false);
                return "TopN Others Reprint";
            }
    ...
    {
        return _childRPGs != null ? _childRPGs.size() : 0;
    }
    ...
    public void shiftShapesBelowHiddenRect(List<RMShape> theShapes, Rect aRect)
    {
        ...
        for (int i = 0, iMax = floatingRects.size(); i < iMax; i++) {
            Rect floatingRect = floatingRects.get(i);
            for (Rect staticRect : staticRects) {
                if (floatingRect.intersectsRect(staticRect)) {
                    ...
                    floatingRect.y += height;
                    floatingRect.height -= height;
                    staticRects.add(floatingRect);
                    i = -1;
                    iMax = floatingRects.size();
                    break;
                }
            }
        }
    ...
    public static class Span implements Comparable<Object> {
        ...
        {
            return MathUtils.lte(start, aValue) && MathUtils.lte(aValue, end);
        }
