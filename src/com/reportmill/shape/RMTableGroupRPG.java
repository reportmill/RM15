...
class RMTableGroupRPG extends RMTableRPG {
    ...
    public RMShape rpgAll()
    {
        ...
        if (_table.getColumnCount() > 1) {
            rshape = makeColumns();
            double x = _tgroup.getX(), y = _tgroup.getY();
    ...
    protected RMTableRPG createPage()
    {
        ...
        page._childTables = _childTables;
        page._groups = _groups;
        return page;
    }
    ...
    protected boolean addChildTableRows(RMGroup aGroup, RMTableRowRPG aParentRPG, RMTableRowRPG theLastRow)
    {
        ...
        List<RMTable> ctables = _tgroup.getChildTables(_table);
        if (ctables == null || ctables.size() == 0) return true;
        RMTable cctable = _childTables.get(_table);

        // Iterate over tables
        for (int i = 0, iMax = ctables.size(); i < iMax; i++) {
            RMTable ctable = ctables.get(i);
            ...
            // If there is a Visible binding and it evaluates to false, continue
            if (ctable.getBinding("Visible") != null) {
                String key = ctable.getBinding("Visible").getKey();
            ...
            if (group == null) {
                _rptOwner.pushDataStack(aGroup);
                group = getGroup(ctable);
                group.setParent(aGroup);
                _rptOwner.popDataStack();
                ...
                if (ctable.getStartingPageBreak() && i > 0 &&
                    ...
                    _lastRow._group = group;
                    _childTables.put(_table, ctable);
                    _groups.put(ctable, group);
                    return false;
                }
            }
            ...
            RMTable oldTable = _table;
            _table = ctable;  //_rptOwner.pushDataStack(group);
            boolean rval = addRows(group, aParentRPG, theLastRow);
            _table = oldTable;  //_rptOwner.popDataStack();

            // If successful, remove table/group from current maps, otherwise add them
            if (rval) {
                _childTables.remove(oldTable);
                _groups.remove(ctable);
            } else {
                _childTables.put(oldTable, ctable);
                _groups.put(ctable, group);
                return false;
            }
            theLastRow = null;
