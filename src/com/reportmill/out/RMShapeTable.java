...
public class RMShapeTable {
     ...
     */
    public double getColWidth(int anIndex)
    {
     ...
     */
    public double getRowHeight(int anIndex)
    {
    ...
    public static RMShapeTable createTable(List<RMShape> theShapes, RMShape topLevel, Rect minTableRect)
    {
        ...
        for (int i = 0; i < shapeCount; i++) {
            ...
            rowStarts[2 * i] = bounds.getY();
            rowStarts[2 * i + 1] = bounds.getMaxY();
            colStarts[2 * i] = bounds.getX();
            colStarts[2 * i + 1] = bounds.getMaxX();
            cellRects[i] = bounds;
        ...
        if (minTableRect != null) {
            rowStarts[2 * shapeCount] = minTableRect.getY();
            rowStarts[2 * shapeCount + 1] = minTableRect.getMaxY();
            colStarts[2 * shapeCount] = minTableRect.getX();
            colStarts[2 * shapeCount + 1] = minTableRect.getMaxX();
        ...
        for (int i = 0; i < shapeCount; i++) {
            ...
            newCell._cshape = shape;
            RMShape s = shape;
            while (s != null && s.getFill() == null) s = s.getParent();
     ...
     */
    static void printOverlapWarning()
    {
        if (!_pow) System.out.println("Warning: overlapping shapes");
        _pow = true;
    }
    ...
    public static class Cell extends RMShape {
         ...
         */
        void setVals(int row, int col, int rspan, int cspan)
        {
            _row = row;
            _col = col;
            _rspan = rspan;
