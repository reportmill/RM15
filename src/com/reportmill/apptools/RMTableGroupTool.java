...
public class RMTableGroupTool<T extends RMTableGroup> extends RMParentShapeTool<T> {
    ...
    public void respondUI(ViewEvent anEvent)
    {
        ...
        if (anEvent.equals("AddPeerMenuItem")) {
            ...
            if (tableGroup.getParentTable(mainTable) != null) {
                addChildTable(tableGroup.getParentTable(mainTable), mainTable, null);
                return;
            }
        ...
        if (anEvent.equals("MoveInMenuItem")) {
            RMTable tableBefore = tableGroup.getPeerTablePrevious(mainTable);
            if (tableBefore == null) return;
    ...
    private static class TGTreeResolver extends TreeResolver<RMTable> {
         ...
         */
        public TGTreeResolver(RMTableGroup aTG)
        {
            _tgroup = aTG;
        }
         ...
         */
        public RMTable getParent(RMTable aTable)
        {
            return _tgroup.getParentTable(aTable);
        }

        /**
         * Whether given object is a parent (has children).
         */
        public boolean isParent(RMTable aTable)
        {
            return _tgroup.getChildTableCount(aTable) > 0;
        }
