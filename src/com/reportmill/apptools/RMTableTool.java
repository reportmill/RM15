...
public class RMTableTool<T extends RMTable> extends RMParentShapeTool<T> implements RMSortPanel.Owner {
    ...
    public void respondUI(ViewEvent anEvent)
    {
        ...
        if (anEvent.equals("AddGroupMenuItem")) {
            ...
            DialogBox dbox = new DialogBox("Add Grouping Key");
            dbox.setQuestionMessage("Enter Grouping Key:");
        ...
        if (anEvent.equals("MoveGroupUpMenuItem")) {
            int loc = _groupingTable.getSelRowIndex();
            if (loc > 0)
    ...
    public void configureGroupingTable(ListCell<RMGrouping> aCell)
    {
        ...
        RMTable table = getTable();
        if (table == null) return;
        RMGrouping grouping = aCell.getItem();
        if (grouping == null) return;
        int row = aCell.getRow(), col = aCell.getCol();

        // Handle column 0
        if (col == 0) {
            String key = grouping.getKey();
            aCell.setText(key);
    ...
    {
        if (_resizeBarIndex < 0)
            return;                                         // If no resize bar selected, just return
