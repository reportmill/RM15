...
public class RMTool<T extends RMShape> extends ViewOwner {
    ...
    {
        return ClassUtils.newInstance(getShapeClass());
    }
    ...
    {
        return getClass() == RMTool.class ? new Label() : super.createUI();
    }
    ...
    {
        return aShape.getDatasetEntity();
    }
     ...
     */
    public RMEditorEvents getEditorEvents()
    {
        return getEditor().getEvents();
    }
    ...
    {
        return getEditor().getSelectedOrSuperSelectedShapes();
    }

    /**
     * Returns the tool for a given shape.
     */
    public RMTool getTool(RMShape aShape)
    {
        return _editor.getTool(aShape);
    }

    /**
     * Returns the tool for a given shape class.
     */
    public RMTool getToolForClass(Class<? extends RMShape> aClass)
    {
        return _editor.getTool(aClass);
    }
    ...
    private Point dropFile(RMShape aShape, ClipboardData aFile, Point aPoint)
    {
        ...
        if (!aFile.isLoaded()) {
            aFile.addLoadListener(f -> dropFile(aShape, aFile, aPoint));
            return aPoint;
        }
    ...
    private void dropImageFile(RMShape aShape, ClipboardData aFile, Point aPoint)
    {
        ...
        if (aShape != editor.getSelPage()) {
            ...
            switch (dbox.showOptionDialog(null, options[0])) {
                ...
                case 0:
                    while (!getTool(aShape).getAcceptsChildren(aShape)) aShape = aShape.getParent();
                    break;

                // Handle Create Texture
                case 1:
                    aShape.setFill(new RMImageFill(imgSrc, true));
        ...
        if (imgShape.getWidth() / editor.getWidth() > .75f || imgShape.getHeight() / editor.getHeight() > .75)
            imgShape.setXY(0, 0);
    ...
    private void dropPDFFile(RMShape aShape, ClipboardData aFile, Point aPoint)
    {
        ...
        while (!getTool(shape).getAcceptsChildren(shape)) shape = shape.getParent();
        ...
        if (pdfShape.getWidth() / editor.getWidth() > .75f || pdfShape.getHeight() / editor.getHeight() > .75)
            pdfShape.setXY(0, 0);
    ...
    private void dropReportFile(RMShape aShape, ClipboardData aFile, Point aPoint)
    {
        ...
        RMNestedDoc ndoc = new RMNestedDoc();
        ndoc.setNestedDoc(doc);
