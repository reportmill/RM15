...
public class RMSelectTool extends RMTool {
    ...
    private List<RMShape> getHitShapes()
    {
        ...
        RMEditor editor = getEditor();
        RMParentShape superShape = editor.getSuperSelectedParentShape();
        if (superShape == null) return Collections.emptyList();
    ...
    public void paintTool(Painter aPntr)
    {
        ...
        aPntr.setColor(new Color(.9, .5));
        aPntr.fill(rect);
        aPntr.setStroke(Stroke.Stroke1);
        aPntr.setColor(new Color(.6, .6));
        aPntr.draw(rect);
    }
    ...
    {
        Rect bnds0 = aShape.getBoundsMarkedDeep();
        bnds0.inset(-4);
