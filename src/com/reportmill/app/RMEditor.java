...
public class RMEditor extends RMViewer implements DeepChangeListener {
     ...
     */
    protected RMShapePaintProps createShapePaintProps()
    {
        return new EditorShapePainterProps();
    }
     ...
     */
    protected RMEditorDnD createDragHelper()
    {
        return new RMEditorDnD(this);
    }
    ...
    public void setSelectedShapes(List<RMShape> theShapes)
    {
        ...
        if (theShapes.size() > 1 && shape == getDoc()) {
            setSuperSelectedShape(theShapes.get(theShapes.size() - 1));
            return;
        }
    ...
    {
        return ListUtils.containsId(_selShapes, aShape);
    }
    ...
    {
        return ListUtils.containsId(_superSelShapes, aShape);
    }
    ...
    {
        return _superSelShapes.get(anIndex);
    }
    ...
    protected Rect getRepaintBoundsForShape(RMShape aShape)
    {
        ...
        if (isSelected(aShape)) bnds.inset(-4, -4);
            ...
            bnds = getTool(aShape).getBoundsSuperSelected(aShape);
            bnds.inset(-16, -16);
        }
     ...
     */
    public RMShape getShapeAtPoint(double aX, double aY)
    {
        return getShapeAtPoint(new Point(aX, aY));
    }
    ...
    public RMShape getChildShapeAtPoint(RMShape aShape, Point aPoint)
    {
        ...
        for (int i = aShape.getChildCount(); i > 0; i--) {
            RMShape child = aShape.getChild(i - 1);
    ...
    {
        return (RMEditorEvents) super.getEvents();
    }

    /**
     * Override to return RMEditorEvents.
     */
    public RMViewerEvents createEvents()
    {
        return new RMEditorEvents(this);
    }
    ...
    private class EditorShapePainterProps extends RMShapePaintProps {
        ...
        {
            return RMEditor.this.getSuperSelectedShape() == aShape;
        }
    }
