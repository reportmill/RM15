...
public class RMEditorUtils {
    ...
    private static void groupShape(RMShape child, RMParentShape gshape)
    {
        ...
        child.setScaleX(child.getScaleX() / gshape.getScaleX());
        child.setScaleY(child.getScaleY() / gshape.getScaleY());
        child.setSkewX(child.getSkewX() - gshape.getSkewX());
        child.setSkewY(child.getSkewY() - gshape.getSkewY());
    ...
    public static void bringToFront(RMEditor anEditor)
    {
        ...
        if (parent == null || anEditor.getSelectedShapeCount() == 0) {
            anEditor.beep();
            return;
        }
    ...
    {
        RMDocument doc = anEditor.getDoc();
        if (anEditor.getSelectedShapeCount() == 0 || doc == null) {
            anEditor.beep();
            return;
        }
     ...
     */
    public static boolean isUnderlined(RMEditor anEdtr)
    {
        return anEdtr.getSelectedOrSuperSelectedShape().isUnderlined();
    }
    ...
    public static void runDatasetKeyPanel(RMEditor anEditor, String aKeyPath)
    {
        ...
        switch (type) {
            case DatasetKeyPanel.TABLE:
                RMTableTool.addTable(anEditor, aKeyPath);
                break;
            case DatasetKeyPanel.GRAPH:
                RMGraphTool.addGraph(anEditor, aKeyPath);
                break;
            case DatasetKeyPanel.LABELS:
                RMLabelsTool.addLabels(anEditor, aKeyPath);
                break;
            case DatasetKeyPanel.CROSSTAB:
                RMCrossTabTool.addCrossTab(anEditor, aKeyPath);
                break;
        }
