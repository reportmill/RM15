...
public class FormatPanel extends RMEditorPane.SupportPane {
    ...
    public void respondUI(ViewEvent anEvent)
    {
        ...
        if (anEvent.equals("NumberFormatText") && anEvent.getStringValue().length() > 0) {
            try {
                ...
                DialogBox dbox = new DialogBox("Invalid Number Format String");
    ...
    {
        RMNumberFormat fmt = aCell.getItem();
        if (fmt == null) return;
    ...
    {
        RMDateFormat fmt = aCell.getItem();
        if (fmt == null) return;
    ...
    {
        return _numberFormats.size();
    }

    /**
     * Returns the preset number format at the given index.
     */
    public RMNumberFormat getNumberFormat(int anIndex)
    {
        return _numberFormats.get(anIndex);
    }
    ...
    {
        return _dateFormats.size();
    }

    /**
     * Returns the preset date format at the given index.
     */
    public RMDateFormat getDateFormat(int anIndex)
    {
        return _dateFormats.get(anIndex);
    }
